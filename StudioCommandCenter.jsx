...
export default function StudioCommandCenter() {
  ...
  const initialNodes = [
    { id: 'node-01', name: 'Node 1', engine: 'Blender', profile: 'GRID_RTX6000-12Q', vramUsed: 6.7, vramTotal: 12.0, gpuUtil: 95, temp: 72, task: 'VFX_BATTLE_012_LIGHT', status: 'HEALTHY' },
    { id: 'node-02', name: 'Node 2', engine: 'Blender', profile: 'GRID_RTX6000-12Q', vramUsed: 7.0, vramTotal: 12.0, gpuUtil: 90, temp: 70, task: 'VFX_BATTLE_012_COMP', status: 'HEALTHY' },
    { id: 'node-03', name: 'Node 3', engine: 'Blender', profile: 'GRID_RTX6000-24Q', vramUsed: 14.3, vramTotal: 24.0, gpuUtil: 92, temp: 78, task: 'VFX_BATTLE_013_GEO', status: 'HEALTHY' },
    { id: 'node-04', name: 'Node 4', engine: 'Unreal Engine', profile: 'GRID_RTX6000-24Q', vramUsed: 17.2, vramTotal: 24.0, gpuUtil: 97, temp: 81, task: 'VFX_BATTLE_014_RENDER', status: 'HEALTHY' },
    { id: 'node-05', name: 'Node 5', engine: 'Unreal Engine', profile: 'GRID_RTX6000-12Q', vramUsed: 8.5, vramTotal: 12.0, gpuUtil: 88, temp: 74, task: 'VFX_BATTLE_015_ANIM', status: 'HEALTHY' },
    { id: 'node-06', name: 'Node 6', engine: 'Unreal Engine', profile: 'GRID_RTX6000-12Q', vramUsed: 8.4, vramTotal: 12.0, gpuUtil: 88, temp: 73, task: 'VFX_BATTLE_015_FX', status: 'HEALTHY' }
  ];

  const [nodes, setNodes] = useState(initialNodes);
  const [alerts, setAlerts] = useState([
    { id: 'a1', severity: 'info', title: 'Grafana Alloy Active', text: 'Scraping metrics & logs from 6 virtualised nodes.', time: 'Just now' }
  ]);
  const [timeline, setTimeline] = useState([
    { id: 't1', type: 'system', title: 'Guardian Core Initialised', desc: 'Showrunner AI Technical Director standing by to protect cluster.', time: '10 mins ago' }
  ]);
  const [chatLog, setChatLog] = useState([
    { id: 'm1', sender: 'assistant', text: 'Salutations. I am the Showrunner AI Technical Director. I monitor GPU telemetry, VRAM overheads, and Arnold/Loki log streams across our 6-node virtual render farm. How can I protect the pipeline today?' }
  ]);
  ...
  const handleInjectCUDA_OOM = () => {
    ...
    setNodes(prev => prev.map(node => 
      node.id === 'node-04' 
        ? { ...node, vramUsed: 24.0, gpuUtil: 100, temp: 88, status: 'WARNING' } 
        : node
    ));

    setAlerts(prev => [
      { id: 'oom-1', severity: 'warning', title: 'VRAM Threshold Breached', text: 'Node 4 GPU memory utilization is at 100%. Critical bounds reached.', time: '0s ago' },
      ...prev
    ]);

    setTimeline(prev => [
      ...prev,
      { id: `t-${Date.now()}-1`, type: 'alert', title: 'Prometheus Alert Fired', desc: 'nvidia_smi_vgpu_fb_used_bytes == nvidia_smi_vgpu_fb_total_bytes on Node 4', time: '0s' }
    ]);
    ...
    setTimeout(() => {
      setNodes(prev => prev.map(node => 
        node.id === 'node-04' 
          ? { ...node, vramUsed: 24.0, gpuUtil: 0, temp: 45, status: 'CRASHED_OOM', task: 'CRASHED (VFX_BATTLE_014_RENDER)' } 
          : node
      ));

      setAlerts(prev => [
        { id: 'oom-2', severity: 'critical', title: 'CUDA Out Of Memory', text: 'CRITICAL error: CUDA_ERROR_OUT_OF_MEMORY: Shot VFX_BATTLE_014 failed on Node 4.', time: '2s ago' },
        ...prev
      ]);
      ...
      setTimeline(prev => [
        ...prev,
        { id: `t-${Date.now()}-2`, type: 'analysis', title: 'Loki Diagnostic Pattern Scan', desc: 'Identified matching traceback: "Arnold GPU device out of memory" in render logs.', time: '2s' }
      ]);
    }, 1500);
    ...
    setTimeout(() => {
      setTimeline(prev => [
        ...prev,
        { id: `t-${Date.now()}-3`, type: 'decision', title: 'Remediation Formulated', desc: 'Correlated VRAM flatline with Loki OOM log signatures. Executing switch to Tiled Rendering Config (64x64 blocks).', time: '4s' }
      ]);
    }, 3500);
    ...
    setTimeout(() => {
      setTimeline(prev => [
        ...prev,
        { id: `t-${Date.now()}-4`, type: 'remediation', title: 'Queue Webhook Dispatched', desc: 'PATCH request sent to queue-manager api updates job renderer config to "tiled".', time: '6s' }
      ]);
    }, 5000);
    ...
    setTimeout(() => {
      setNodes(prev => prev.map(node => 
        node.id === 'node-04' 
          ? { ...node, vramUsed: 9.2, gpuUtil: 84, temp: 73, status: 'HEALTHY', task: 'VFX_BATTLE_014_RENDER (TILED_64x64)' } 
          : node
      ));
      ...
      setTimeline(prev => [
        ...prev,
        { id: `t-${Date.now()}-5`, type: 'resolution', title: 'System Restored', desc: 'Node 4 healthy under tiled render load. Core allocations reduced from 24GB to 9.2GB VRAM.', time: '8s' }
      ]);

      setChatLog(prev => [
        ...prev,
        { id: `chat-${Date.now()}`, sender: 'assistant', text: '🚨 [AUTO-REMEDIATION REPORT] I detected an extreme CUDA VRAM saturation on Node 4 during render job VFX_BATTLE_014. By scanning the Loki log streams and confirming "CUDA_ERROR_OUT_OF_MEMORY", I executed a configuration swap via our studio queue manager. Node 4 is now fully operational rendering with 64x64 Tiles, bringing active VRAM down to 9.2GB.' }
      ]);
  ...
  const handleInjectDeadlock = () => {
    ...
    setAlerts(prev => [
      { id: 'dl-1', severity: 'warning', title: 'Node Deadlock Warning', text: 'Node 3 CPU usage at 100% while GPU utilisation is 0%. Possible execution hang.', time: '0s ago' },
      ...prev
    ]);

    setTimeline(prev => [
      ...prev,
      { id: `t-${Date.now()}-dl1`, type: 'alert', title: 'Deadlock Metric Sig Fired', desc: 'Prometheus rule: node_cpu_utilization == 100 && gpu_utilization == 0 active.', time: '0s' }
    ]);
    ...
    setTimeout(() => {
      setTimeline(prev => [
        ...prev,
        { id: `t-${Date.now()}-dl2`, type: 'analysis', title: 'Pyroscope Profile Scrape', desc: 'Active continuous thread traces show prolonged lock durations on parallel Arnold CPU synchronization spans.', time: '2s' }
      ]);
    }, 1500);
    ...
    setTimeout(() => {
      setTimeline(prev => [
        ...prev,
        { id: `t-${Date.now()}-dl3`, type: 'decision', title: 'Force Process Reboot Triggered', desc: 'Deadlock confirmed via continuous profiling lock waits. Command dispatched to drop node and reboot hypervisor daemon.', time: '3.5s' }
      ]);
    }, 3500);
    ...
    setTimeout(() => {
      setTimeline(prev => [
        ...prev,
        { id: `t-${Date.now()}-dl4`, type: 'remediation', title: 'Hard Restart Webhook Fired', desc: 'POST request sent to hypervisor manager: /nodes/node-03/restart executed.', time: '5.0s' }
      ]);
    }, 5000);
    ...
    setTimeout(() => {
      setNodes(prev => prev.map(node => 
        node.id === 'node-03' 
          ? { ...node, vramUsed: 0.0, gpuUtil: 0, temp: 35, status: 'HEALTHY', task: 'IDLE (QUEUED_FOR_GEO)' } 
          : node
      ));
  ...
  const handleSendChat = (textToSend = chatInput) => {
    ...
    const userMessage = { id: `msg-${Date.now()}`, sender: 'user', text: cleanText };
    ...
    setTimeout(() => {
      ...
      } else if (textLower.includes('clear alerts') || textLower.includes('reset')) {
        setNodes(initialNodes);
        setAlerts([{ id: 'a1', severity: 'info', title: 'Grafana Alloy Active', text: 'Scraping metrics & logs from 6 virtualised nodes.', time: 'Just now' }]);
        responseText = "🔄 Reset request processed. Telemetry logs and node status indicators have been restored to default healthy baselines.";
      } else {
      ...
      setChatLog(prev => [...prev, { id: `bot-${Date.now()}`, sender: 'assistant', text: responseText }]);
    }, 1000);
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight text-white flex items-center gap-2">
              SHOWRUNNER AI <span className="text-xs bg-cyan-500/15 text-cyan-400 border border-cyan-500/20 px-2 py-0.5 rounded font-mono">BETA v1.2</span>
            </h1>
            <p className="text-xs text-slate-500">Autonomous VFX Render Farm Guardian & Remediation Dashboard</p>
          </div>
        </div>

        {/* Demo Execution / SRE Action Controls */}
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-500 hidden md:inline font-mono">DEMO INJECTION SUITE:</span>
          <button 
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span> 6 Online</span>
              </div>
            </div>
                ...
                return (
                  <div 
                    ...
                    className={`rounded-xl border p-4 transition-all duration-300 bg-slate-900/50 relative overflow-hidden ${
                      isCrashed ? 'border-red-500/40 shadow-lg shadow-red-500/5 bg-red-950/5' :
                      isDeadlocked ? 'border-amber-500/40 shadow-lg shadow-amber-500/5 bg-amber-950/5' :
                      isWarning ? 'border-amber-500/40 shadow-lg shadow-amber-500/5' :
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-bold text-white text-sm flex items-center gap-2">
                          {node.name}
                          <span className="text-[10px] font-mono text-slate-500">{node.profile.split('-')[1]}</span>
                        </h3>
                        <p className="text-xs text-slate-400">{node.engine}</p>
                      </div>

                      <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase ${
                        isHealthy ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/25' :
                        isCrashed ? 'bg-red-500/10 text-red-400 border border-red-500/25 animate-pulse' :
                    ...
                    <div className="space-y-1 mb-3">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-mono text-white">{node.gpuUtil}%</span>
                      </div>
                      <div className="h-1.5 bg-slate-950 rounded-full overflow-hidden">
                        <div 
                          className={`h-full transition-all duration-500 ${
                            isCrashed ? 'bg-red-500' :
                            node.gpuUtil > 90 ? 'bg-cyan-400' :
                            'bg-slate-700'
                          }`}
                    ...
                    <div className="space-y-1 mb-3">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-mono text-white">
                          {node.vramUsed.toFixed(1)} / {node.vramTotal.toFixed(1)} GB
                        </span>
                      </div>
                      <div className="h-1.5 bg-slate-950 rounded-full overflow-hidden">
                        <div 
                          className={`h-full transition-all duration-500 ${
                            isCrashed ? 'bg-red-500' :
                            (node.vramUsed / node.vramTotal) > 0.8 ? 'bg-amber-500' :
                            'bg-cyan-500'
                          }`}
                    ...
                    <div className="pt-2 border-t border-slate-950/80 flex items-center justify-between gap-3 text-[11px]">
                      <div className="flex items-center gap-1 text-slate-400 font-mono">
                        <ThermometerIcon className={`h-3 w-3 ${node.temp > 80 ? 'text-red-400' : 'text-slate-400'}`} />
          ...
          <div className="border border-slate-900 rounded-xl bg-slate-900/20 p-5 flex-1 flex flex-col min-h-[300px]">
            <div className="flex items-center justify-between pb-3 border-b border-slate-900 mb-4">
              ...
              <div className="flex items-center gap-4 text-xs font-mono">
                <span className="text-slate-400">Scrape Rate: <span className="text-cyan-400">10s</span></span>
                <span className="text-slate-400">API Endpoint: <span className="text-slate-400">/loki/api/v1/push</span></span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 flex-1">
              ...
              <div className="bg-slate-950/80 border border-slate-900/60 rounded-xl p-4 flex flex-col justify-between">
                <div>
                  <span className="text-[10px] text-cyan-500 font-mono tracking-wider uppercase block mb-1">PROMETHEUS TIME-SERIES</span>
                  <h3 className="text-sm font-bold text-white mb-3">Cluster VRAM vs Render Latency</h3>
                </div>
                ...
                <div className="h-32 w-full flex items-end gap-1.5 relative py-2">
                  <div className="absolute inset-0 flex flex-col justify-between text-[9px] text-slate-600 font-mono pointer-events-none border-b border-slate-900 pb-1">
                    <div className="border-t border-slate-900/50 w-full pt-1">24 GB (VRAM peak)</div>
                    <div className="border-t border-slate-900/50 w-full pt-1">12 GB (Average)</div>
                    <div>0 GB</div>
                  </div>
                  ...
                  <div className={`flex-1 transition-all duration-500 rounded-t ${activeAlertCount > 0 ? 'bg-red-500 h-full animate-pulse' : 'bg-cyan-500 h-[75%]'}`}></div>
                ...
                <div className="flex items-center gap-4 text-xs font-mono text-slate-500 pt-3 border-t border-slate-900/40">
                  <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-cyan-500"></span> Mean Latency: 12.4s</span>
                  <span className="flex items-center gap-1.5"><span className={`h-2 w-2 rounded-full ${activeAlertCount > 0 ? 'bg-red-500 animate-ping' : 'bg-slate-600'}`}></span> Loki Alerts: {activeAlertCount} active</span>
                </div>
              </div>
              ...
              <div className="bg-slate-950/80 border border-slate-900/60 rounded-xl p-4 flex flex-col justify-between">
                <div>
                  <span className="text-[10px] text-red-400 font-mono tracking-wider uppercase block mb-1">LIVE LOKI INGEST STREAM</span>
                  <h3 className="text-sm font-bold text-white mb-2">Ingestion Warnings & Failures</h3>
                </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-2 max-h-[140px] pr-2 font-mono text-[11px] leading-relaxed select-text mt-2">
                  {alerts.map(alert => {
                    ...
                    return (
                      <div key={alert.id} className={`p-2 rounded border leading-relaxed ${
                        isCrit ? 'bg-red-500/5 border-red-500/25 text-red-400' :
                        isWarn ? 'bg-amber-500/5 border-amber-500/25 text-amber-400' :
                        'bg-slate-900/40 border-slate-900 text-slate-400'
                      }`}>
                        <div className="flex justify-between font-bold mb-0.5 text-[10px]">
                          <span className="uppercase tracking-widest">{alert.severity}</span>
                          <span className="opacity-60 font-normal">{alert.time}</span>
                        </div>
                        <p className="font-semibold text-slate-200">{alert.title}</p>
                        <p className="opacity-80 text-[10px] mt-0.5">{alert.text}</p>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="border border-slate-900 rounded-xl bg-slate-900/20 p-5 flex flex-col h-[320px]">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin">
              {timeline.map((event) => {
                ...
                if (event.type === 'alert') badgeColor = 'bg-red-500/10 border-red-500/30 text-red-400 animate-pulse';
                if (event.type === 'analysis') badgeColor = 'bg-purple-500/10 border-purple-500/30 text-purple-400';
                if (event.type === 'decision') badgeColor = 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400';
                if (event.type === 'remediation') badgeColor = 'bg-amber-500/10 border-amber-500/30 text-amber-400';
                if (event.type === 'resolution') badgeColor = 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400';

                return (
                  <div key={event.id} className="relative pl-5 border-l border-slate-900">
                    {/* Node Dot */}
                    <div className="absolute -left-1.5 top-1.5 h-3 w-3 rounded-full bg-slate-950 border border-slate-800 flex items-center justify-center">
                      <span className={`h-1.5 w-1.5 rounded-full ${
                        event.type === 'alert' ? 'bg-red-500 animate-ping' :
                        event.type === 'resolution' ? 'bg-emerald-400' :
                        'bg-cyan-500'
                      }`} />
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <span className="text-[10px] text-slate-600 font-mono">{event.time}</span>
                    </div>

                    {/* Event Content */}
                    <h4 className="text-xs font-semibold text-slate-200">{event.title}</h4>
                    <p className="text-[10px] text-slate-400 leading-normal mt-0.5">{event.desc}</p>
                  </div>
                );
              })}
          ...
          <div className="border border-slate-900 rounded-xl bg-slate-900/20 p-5 flex flex-col flex-1 min-h-[380px]">
              ...
              {chatLog.map(msg => (
                <div key={msg.id} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                  <span className="text-[10px] text-slate-600 mb-1 font-mono uppercase tracking-wider">
                    {msg.sender === 'user' ? 'VFX Lead (Director)' : 'Showrunner AI Agent'}
                  </span>
            ...
            <div className="mb-3 shrink-0">
              <p className="text-[9px] text-slate-600 font-mono uppercase tracking-wider mb-1.5">Quick Directives:</p>
              <div className="flex flex-wrap gap-1.5">
                ...
                <button 
                  onClick={() => handleSendChat('Prepare morning briefing for VFX lead')}
                  className="px-2 py-1 rounded bg-slate-950 border border-slate-900 text-[10px] text-cyan-400 hover:bg-slate-900 hover:border-slate-800 transition"
                >
